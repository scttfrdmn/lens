...
func runLaunch(environment, instanceType, customAMI, idleTimeout, profile, region, availabilityZone string, dryRun bool, connectionMethod, subnetType string, createNatGateway bool, useSpot bool, spotMaxPrice, spotType, s3Bucket, s3SyncPath string) error {
	...
	if dryRun {
		return executeDryRun(ctx, env, profile, region, availabilityZone, idleTimeout, connectionMethod, subnetType, createNatGateway, useSpot, spotMaxPrice, spotType, s3Bucket)
	}
...
func displayLaunchWarnings(connectionMethod, subnetType string, createNatGateway bool, useSpot bool, s3Bucket string) {
	...
	if subnetType == subnetTypePrivate && !createNatGateway {
		...
		out.List("RStudio extensions may not work")
...
func executeDryRun(ctx context.Context, env *config.Environment, profile, region, availabilityZone, idleTimeout, connectionMethod, subnetType string, createNatGateway bool, useSpot bool, spotMaxPrice, spotType, s3Bucket string) error {
	...
	printDryRunConfiguration(env, actualRegion, profile, region, idleTimeout, connectionMethod, subnetType, createNatGateway, useSpot, spotMaxPrice, spotType, s3Bucket, keyName)
...
func prepareInstanceImage(ctx context.Context, ec2Client *aws.EC2Client, env *config.Environment, region, customAMI string, idleTimeoutSeconds int, s3Bucket, s3SyncPath string) (string, string, error) {
	...
	userData, err := rstudioconfig.GenerateUserData(env, idleTimeoutSeconds, s3Bucket, s3SyncPath)
	if err != nil {
...
func launchAndWaitForInstance(ctx context.Context, ec2Client *aws.EC2Client, ssmClient *aws.SSMClient, env *config.Environment, subnet *aws.SubnetInfo, securityGroup *aws.SecurityGroupInfo, amiID, userData string, keyInfo *aws.KeyPairInfo, instanceProfile *aws.InstanceProfileInfo, useSpot bool, spotMaxPrice, spotType string) (*types.Instance, error) {
	...
	out.Blank()
	out.Status("Installing RStudio Server")
...
func displayInstanceInfo(instance *types.Instance, env *config.Environment, subnet *aws.SubnetInfo, keyInfo *aws.KeyPairInfo, connectionMethod, subnetType, profile string) error {
	...
	out.Blank()
	out.Complete("RStudio Server launched successfully!")
	out.Blank()
	...
	out.Blank()
	out.Info("RStudio Server will be available at: http://localhost:8787")
	out.Status("Please wait 2-3 minutes for RStudio Server to complete installation")
...
func printDryRunConfiguration(env *config.Environment, actualRegion, profile, region, idleTimeout, connectionMethod, subnetType string, createNatGateway bool, useSpot bool, spotMaxPrice, spotType, s3Bucket, keyName string) {

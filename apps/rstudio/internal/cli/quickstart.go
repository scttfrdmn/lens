...
func NewQuickstartCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "quickstart",
		Short: "Launch RStudio Server with smart defaults (fastest way to get started)",
		Long: `The quickstart command launches an RStudio Server instance with sensible defaults,
...
Smart defaults used:
- Environment: default (R with common statistical packages)
...
func runQuickstart(cmd *cobra.Command, args []string) error {
	...
	out.Header("🚀 RStudio Server Quickstart")
	out.Blank()
	...
	out.KeyValue("Environment", "Default (R + statistical packages)")
	...
	out.Progress("Launching your RStudio Server environment...")
	out.Blank()
...
func generateQuickstartName() string {
	now := time.Now()
	return fmt.Sprintf("rstudio-quickstart-%s", now.Format("2006-01-02-150405"))
}

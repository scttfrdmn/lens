...
func NewStatusCmd() *cobra.Command {
	return &cobra.Command{
		...
		Short: "Show VSCode Server instance status and details",
...
func runStatus(instanceID string) error {
	...
	// Display status
	fmt.Printf("VSCode Server Instance Status: %s\n", instanceID)
	...
	if instance.TunnelPID > 0 {
		fmt.Printf("Tunnel:          Active (PID %d)\n", instance.TunnelPID)
		fmt.Printf("VSCode Server:   http://localhost:8080\n")
	} else {
		fmt.Printf("Tunnel:          Not active\n")
	}

...
Use --no-reboot to create the AMI without rebooting (not recommended for production).`,
	...
	cmd.Flags().StringVarP(&name, "name", "n", "", "AMI name (default: aws-vscode-<environment>-<timestamp>)")
...
func runCreateAMI(instanceID string, name string, noReboot bool) error {
	...
	if name == "" {
		...
		name = fmt.Sprintf("aws-vscode-%s-%s", instance.Environment, timestamp)
	}
	...
	amiID, err := ec2Client.CreateAMI(ctx, instanceID, name, fmt.Sprintf("aws-vscode %s environment", instance.Environment), noReboot)
	if err != nil {

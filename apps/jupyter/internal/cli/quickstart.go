...
func NewQuickstartCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "quickstart",
		Short: "Launch Jupyter Lab with smart defaults (fastest way to get started)",
		Long: `The quickstart command launches a Jupyter Lab instance with sensible defaults,
...
Smart defaults used:
- Environment: default (Python with common data science packages)
...
func runQuickstart(cmd *cobra.Command, args []string) error {
	...
	out.Header("🚀 Jupyter Lab Quickstart")
	out.Blank()
	...
	out.KeyValue("Environment", "Default (Python + data science tools)")
	...
	out.Progress("Launching your Jupyter Lab environment...")
	out.Blank()
...
func generateQuickstartName() string {
	now := time.Now()
	return fmt.Sprintf("jupyter-quickstart-%s", now.Format("2006-01-02-150405"))
}

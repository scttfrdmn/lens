...
func launchAndWaitForInstance(ctx context.Context, ec2Client *aws.EC2Client, env *config.Environment, subnet *aws.SubnetInfo, securityGroup *aws.SecurityGroupInfo, amiID, userData string, keyInfo *aws.KeyPairInfo, instanceProfile *aws.InstanceProfileInfo) (*types.Instance, error) {
	...
	if err := waitForJupyterReady(ctx, instance); err != nil {
...
// waitForJupyterReady polls Jupyter Lab until it's ready to accept connections
func waitForJupyterReady(ctx context.Context, instance *types.Instance) error {
	...
	if result.Ready {
		fmt.Printf("✓ Jupyter Lab is ready! (took %v)\n", result.ElapsedTime.Round(time.Second))
		return nil
	}

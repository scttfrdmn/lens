...
func NewListAMIsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list-amis",
		Short: "List custom lens-jupyter AMIs",
		Long: `List all AMIs created by lens-jupyter CLI.
...
func runListAMIs(region string) error {
	...
	if len(amis) == 0 {
		...
		fmt.Printf("\nCreate an AMI with: lens-jupyter create-ami INSTANCE_ID\n")
		return nil
	}
	...
	fmt.Printf("Custom lens-jupyter AMIs in region %s:\n\n", ec2Client.GetRegion())
	...
	fmt.Printf("  lens-jupyter launch --env ENVIRONMENT --ami AMI_ID\n")
